...
assert.match(inferenceRouteSource, /applyToRunningSandboxes/, 'inference route must support applying a default endpoint to running sandboxes')
assert.match(inferenceRouteSource, /readRunningSandboxes/, 'inference route must discover running sandboxes for default endpoint rollout')
assert.match(inferenceRouteSource, /applySandboxInferenceProfile/, 'inference route must live-apply the default endpoint to running sandboxes')
assert.match(inferencePanelSource, /INFERENCE ENDPOINTS/, 'settings UI must expose inference endpoint configuration')
assert.match(inferencePanelSource, /https:\/\/inference-api\.nvidia\.com\/v1/, 'settings UI must provide the NVIDIA inference API preset')
assert.match(inferencePanelSource, /aws\/anthropic\/claude-opus-4-5/, 'settings UI must include the requested NVIDIA inference model preset')
assert.match(inferencePanelSource, /method: "DELETE"/, 'settings UI must expose provider deletion')
assert.match(inferencePanelSource, /Set as Default Inference Endpoint/, 'settings UI must expose a default endpoint rollout action')
assert.match(inferencePanelSource, /This will change the default inference endpoint to ALL running sandboxes and newly created sandboxes\./, 'default endpoint action must warn about all running and new sandboxes')
assert.match(inferencePanelSource, /applyToRunningSandboxes: true/, 'settings UI must request default endpoint rollout after confirmation')
