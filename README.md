...
This dashboard is validated against the current NVIDIA NemoClaw repo and the OpenShell version range declared in NemoClaw's `nemoclaw-blueprint/blueprint.yaml`, not the older April 2026 point releases. Current NemoClaw `main` pins OpenShell exactly to `0.0.99`, so the bundled refresh helper defaults to:

- OpenShell installer release: `v0.0.99` (`OPENSHELL_VERSION=v0.0.99`)
...
That helper defaults to `OPENSHELL_VERSION=v0.0.99`, `NEMOCLAW_INSTALL_REF=main`, and `OPENCLAW_VERSION=2026.7.1`.
